<script lang="ts">
  ...
  import { decompressFromToon } from "./agents/compression/toon";
  ...
  import {
    ...
    ENCODING_OPTIONS,
    type EncodingOption,
    buildFileNameForFormat,
  } from "./agents/import-export/encoding";
  import {
    getRecentFiles,
    addRecentFile,
    removeRecentFile,
  ...
  type RawViewComponentType =
    typeof import("./components/RawView.svelte").default;
  ...
  let encodingDialogResolver:
    | ((value: EncodingSelection | null) => void)
    | null = null;
  ...
  let saveAsDialogResolver: ((value: SaveAsSelection | null) => void) | null =
    null;
  ...
  async function ensureRawViewLoaded() {
    ...
    rawViewLoadPromise = import("./components/RawView.svelte")
      .then((module) => {
  ...
  $: splitViewState = splitViewTabId
    ? buildSplitViewState(splitViewTabId)
    : null;
  ...
  function handleSaveAsDialogConfirm() {
    ...
    if (
      saveAsDialogState.canBrowseDirectory &&
      !saveAsDialogState.directoryPath.trim()
    ) {
      return;
    }
    closeSaveAsDialog({
  ...
  function handleSaveAsDialogFileNameChange(
    event: CustomEvent<{ value: string }>
  ) {
    updateSaveAsDialogState({ fileName: event.detail.value });
  }

  function handleSaveAsDialogEncodingChange(
    event: CustomEvent<{ value: string }>
  ) {
    updateSaveAsDialogState({ encoding: event.detail.value });
  }

  function handleSaveAsDialogDirectoryChange(
    event: CustomEvent<{ value: string }>
  ) {
  ...
  async function handleSaveAsDialogBrowse() {
    ...
    try {
      ...
      const selected = await open({
        ...
        multiple: false,
        defaultPath:
          saveAsDialogState.directoryPath || lastSaveDirectory || undefined,
      });
  ...
  function requestSaveAsOptions(
    tab: Tab,
    format: "json" | "csv" | "xml" | "toon"
  ): Promise<SaveAsSelection | null> {
    const initialFileName = buildFileNameForFormat(tab.name, format);
    const initialDirectory =
      extractDirectoryPath(tab.filePath) || lastSaveDirectory;
    saveAsDialogState = {
      format,
  ...
  function requestImportChoice(pending: PendingImport): Promise<ImportChoice> {
    importDialogFileName =
      pending.file instanceof File ? pending.file.name : pending.file.name;
  ...
  async function handleClipboardImport(data: TableData) {
    try {
      ...
      console.error(
        "[handleClipboardImport] 붙여넣기 데이터 처리 실패:",
        error
      );
      alert(
        `붙여넣기 데이터를 불러오지 못했습니다: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  async function handleTabContextAction(
    targetTabId: string,
    ...
    try {
      ...
      alert(
        `데이터 처리 중 오류가 발생했습니다: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
      ...
      if (!isUpdatingFromTab && activeTabId) {
        ...
        if (activeTab) {
          ...
          if (value && value.rows && Array.isArray(value.rows)) {
            ...
            activeTab.data = activeTab.isStreamingMode
              ? value
  ...
  function createNewTab(
    ...
    const { payload, compressed } = buildTabDataPayload(
      fileData,
  ...
  function createEmptyTab() {
    ...
    const tabId = createNewTab(
      { path: "untitled.json", name: "untitled.json" },
      emptyData,
      "json"
    );
  ...
  function switchToTab(tabId: string) {
    ...
    try {
      ...
      console.error(`[switchToTab] 압축 해제 실패: ${tab.name}`, {
        ...
        errorStack:
          decompressError instanceof Error ? decompressError.stack : undefined,
        isCompressed: tab.isCompressed,
        dataType: typeof tab.data,
        dataLength: typeof tab.data === "string" ? tab.data.length : undefined,
      });
      throw new Error(
        `탭 데이터 압축 해제 실패 (${tab.name}): ${decompressError instanceof Error ? decompressError.message : "Unknown error"}`
      );
    }
  ...
  function updateActiveEncodingState() {
    ...
    canChangeEncoding =
      activeTab.fileFormat === "csv" && Boolean(activeTab.binarySource);
  }
  ...
  async function handleEncodingChangeRequest(newEncoding: string) {
    ...
    try {
      ...
      const fileRef =
        tab.file ??
        (tab.filePath
          ? { path: tab.filePath, name: tab.name }
          : { path: "", name: tab.name });
      ...
      showProgress = false;
      alert(
        `인코딩 변경 실패: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  async function processInputFile(
    ...
    try {
      ...
      await integrateImportedData({
        ...
        binarySource:
          importOptions?.binarySource ?? result.binarySource ?? null,
  ...
  async function handleFileDrop(file: File | { path: string; name: string }) {
    ...
    if (succeeded && isTauriApp && "path" in file && file.path) {
  ...
  async function handleExport(format: "json" | "csv" | "xml" | "toon") {
    try {
      ...
      alert(
        `내보내기 실패: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
          ...
          if (item.kind === "file") {
            try {
              ...
              if (entry && entry.isFile) {
                ...
                console.log("[handleDrop] File 객체 생성:", {
                  name: file.name,
                  size: file.size,
                });
...
>
  ...
  {/if}
  <Toolbar
    isStreamingMode={activeTabId
      ? (tabs.find((t) => t.id === activeTabId)?.isStreamingMode ?? false)
      : false}
    on:newTab={(e) => {
  ...
  <main class="main-content">
    ...
    {:else}
      <div class="stats-controls-container">
        <StatsPanel {data} {tableState} {tableViewRef} />
  ...
  <ProgressBar {progress} message={progressMessage} visible={showProgress} />
  <SettingsDialog
    isOpen={showSettings}
    onClose={() => {
      showSettings = false;
  ...
  <StatusBar
    ...
    fileSize={activeTabId
      ? tabs.find((t) => t.id === activeTabId)?.fileSize
      : undefined}
    isStreamingMode={activeTabId
      ? (tabs.find((t) => t.id === activeTabId)?.isStreamingMode ?? false)
      : false}
...
<style>
  ...
  .recent-file-item:hover {
    ...
    &:has(.recent-file-remove:hover) {
      cursor: pointer;
      position: relative;
      opacity: 0.25;
    }

    &:not(:has(.recent-file-remove:hover)) {
      opacity: 1;
