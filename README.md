...
### 🎨 뷰 및 표시
- ⚡ **대용량 데이터 처리** - 100,000건 이상의 데이터 지원
...
2. **데이터 편집**
   - ✏️ 셀을 클릭하여 직접 편집
   - ⏎ `Enter` 키로 다음 행의 같은 열로 이동
   - ⇧ `Shift+Enter` 키로 이전 행의 같은 열로 이동
   - ⇄ `Tab` 키로 다음 열로 이동, `Shift+Tab`으로 이전 열로 이동
...
대용량 파일을 효율적으로 처리하기 위한 스트리밍 모드가 지원됩니다.

### ✨ 스트리밍 모드란?

스트리밍 모드는 파일 크기가 설정된 임계값(기본값: 설정에서 변경 가능)을 초과할 때 자동으로 활성화됩니다. 이 모드에서는 파일을 청크 단위로 읽어 메모리 사용량을 최소화하면서 대용량 파일을 처리할 수 있습니다.

### 🎯 스트리밍 모드 특징

- **자동 활성화**: 파일 크기가 임계값을 초과하면 자동으로 스트리밍 모드로 전환
- **메모리 효율적**: 전체 파일을 메모리에 로드하지 않고 청크 단위로 처리
- **진행률 표시**: 파싱 진행 상황을 실시간으로 확인 가능
- **상태 표시**: StatusBar에 스트리밍 모드 아이콘 표시

### 🔍 스트리밍 모드에서의 검색

스트리밍 모드에서는 성능 최적화를 위해 검색 동작이 다르게 작동합니다:

- **일반 모드**: 입력 즉시 검색 실행
- **스트리밍 모드**: 다음 경우에만 검색 실행
  - `Enter` 키 입력
  - `&` 문자 입력 (복합 조건 검색 시)
  - 검색창 포커스 해제 (blur)

이렇게 하면 대용량 데이터에서 타이핑할 때마다 검색이 실행되어 발생할 수 있는 성능 저하를 방지할 수 있습니다.

### ⚙️ 스트리밍 임계값 설정

설정 다이얼로그에서 스트리밍 모드 임계값을 변경할 수 있습니다:
- 기본값: 10MB (10,240 KB)
- 범위: 1MB ~ 100MB
- 파일 크기가 이 값을 초과하면 자동으로 스트리밍 모드로 전환

### 📝 지원 형식

스트리밍 모드는 다음 형식에서 지원됩니다:
- ✅ **JSON**: 배열 형태의 JSON 파일 (스트리밍 파싱)
- ✅ **CSV**: 대용량 CSV 파일 (PapaParse 스트리밍)
- ✅ **XML**: XML 파일 (청크 단위 처리)
- ❌ **TOON**: 압축 형식은 스트리밍 모드 미지원
...
column$=값          // 값으로 끝남
column*=값          // 값이 포함됨
!column=값          // 값과 같지 않음 (부정)
!column^=값         // 값으로 시작하지 않음
!column$=값         // 값으로 끝나지 않음
!column*=값         // 값이 포함되지 않음
```
- 특정 열에서만 검색합니다
- CSS 속성 선택자 스타일의 검색 연산자를 지원합니다
- 정규식 버튼이 켜져있으면 속성 검색도 정규식으로 처리됩니다
- 예: `name=John` - name 열에서 "John"과 정확히 일치하는 값
- 예: `email^=user` - email 열에서 "user"로 시작하는 값
- 예: `domain$=.com` - domain 열에서 ".com"으로 끝나는 값
...
// CSS 속성 선택자 스타일 검색
name^=John          // "John"으로 시작
email$=.com         // ".com"으로 끝남
title*=test         // "test"가 포함됨
!status=active      // "active"가 아님
!name^=Admin        // "Admin"으로 시작하지 않음

// 정규식과 함께 사용 (정규식 버튼 활성화)
name^=^[A-Z]        // 대문자로 시작 (정규식)
email$=\.(com|net)$ // .com 또는 .net으로 끝남 (정규식)
