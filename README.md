...
### 데이터 Import/Export
- JSON 파일 업로드
- CSV 파일 import (JSON으로 자동 변환)
- 클립보드에서 붙여넣기 (JSON/CSV)
- JSON/CSV 형식으로 export
- 로컬스토리지 자동 저장
- 데이터 병합 기능

### 뷰 및 표시
- 빠른 뷰 형성 (10000건 이상의 대용량 데이터 지원)
- 가상 스크롤링으로 성능 최적화
- Raw/Pretty JSON 뷰 전환
- 데이터 통계 정보 표시
- 2층 헤더 구조 지원 (Nested JSON)
- 인덱스 열 왼쪽 고정 (가로 스크롤 시에도 고정)
...
- **열 변환 함수**: JavaScript 함수를 사용하여 열 데이터 일괄 변환

### 테이블 조작
- **열 추가/삭제**: 오른쪽 클릭 메뉴에서 열 추가/삭제
- **행 추가/삭제**: 오른쪽 클릭 메뉴에서 행 추가/삭제
- **열 너비 조정**: 헤더 오른쪽 가장자리를 드래그하여 조정
- **열 정렬**: 헤더 클릭으로 오름차순/내림차순 정렬

### 필터 기능
- 열 헤더의 필터 버튼 클릭으로 필터 열기
- 전체 데이터에서 고유 값 목록 표시 (최대 20개)
- 실시간 필터링 (입력 시 자동 적용)
...
3. **데이터 저장**
   - 툴바의 "다운로드" 버튼으로 JSON/CSV 형식으로 저장
   - 자동으로 로컬스토리지에 저장됨
